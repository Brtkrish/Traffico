...
v5 adds phase stability/hysteresis on top of v4 phase effectiveness rather than repeatedly choosing the larger
...
- requires a stronger opposing demand before switching, reducing rapid phase oscillation;
- keeps a hard maximum red-wait limit as a final starvation guard.
...
OPPOSING_MIN_DEMAND = 5.0
SWITCH_SCORE_MARGIN = 10.0
QUEUE_CLEAR_FRACTION = 0.25
...
def should_switch(
    ...
    # Stable handoff rule: require a clear, sustained advantage on the opposite side
    # before switching. This reduces rapid NS<->EW oscillation and yellow overhead.
    if (
    ...
    if current_queue <= LOW_QUEUE_THRESHOLD and opposite_metrics["halting"] >= 3 and opposite_score >= OPPOSING_MIN_DEMAND:
        return True, "LOW_CURRENT_QUEUE"

    return False, "HOLD"


def run(gui=False, tripinfo_output="adaptive_v5_tripinfo.xml", max_steps=MAX_STEPS):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tripinfo-output",
        default="adaptive_v5_tripinfo.xml",
