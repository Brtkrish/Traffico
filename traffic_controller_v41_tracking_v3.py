...
with st.sidebar.expander("Tracking / Queue Parameters"):
    st.caption("Stock videos use different camera perspectives, so stop thresholds are camera-specific.")
...
# Camera-specific motion thresholds. Perspective differs substantially between
# the four stock videos, so one pixel-speed threshold is not appropriate.
STOP_SPEED_PX_PER_SEC = {
    "North": 20.0,
    "South": 24.0,
    "East": 18.0,
    "West": 14.0,
...
# A track is counted as a real queue member only after it has remained stopped
# for several seconds/observations.
QUEUE_STOP_CONFIRMATIONS = 7
QUEUE_CLEAR_CONFIRMATIONS = 2
...
def update_centroid_tracks(direction, detections, now_ts):
    ...
    for det in detections:
        ...
        if best_id is None:
            ...
            if tr["age"] >= MIN_TRACK_AGE:
                if tr["speed"] <= STOP_SPEED_PX_PER_SEC.get(direction, 18.0):
                    ...
                    if tr["stopped"]:
                        tr["queue_confirmations"] += 1
                        tr["queue_clear_count"] = 0

                        if tr["queue_confirmations"] >= QUEUE_STOP_CONFIRMATIONS:
                            tr["in_queue"] = True
                else:
                    tr["moving_count"] += 1
                    tr["low_speed_count"] = 0
                    tr["queue_confirmations"] = 0

                    if tr["moving_count"] >= MOVING_CONFIRMATIONS:
                        tr["stopped"] = False

                    if tr["in_queue"]:
                        tr["queue_clear_count"] += 1
                        if tr["queue_clear_count"] >= QUEUE_CLEAR_CONFIRMATIONS:
                            tr["in_queue"] = False
                            tr["queue_clear_count"] = 0
    ...
    # Queue is deliberately more conservative than "stopped":
    # a vehicle must remain stopped long enough to be considered a persistent
    # queue member.
    queue_estimate = min(len(persistent_queue), len(mature))
    ...
    st.session_state.track_metrics[direction] = {
        ...
        "persistent_queue": queue_estimate,
    }
    ...
    if do_inference:
        for direction, cap in st.session_state.caps.items():
            ...
            for det in tracked:
                ...
                track_state = st.session_state.tracks[direction].get(det["track_id"], {})
                status = (
                    "QUEUE"
                    if track_state.get("in_queue", False)
                    else ("STOP" if det["stopped"] else "MOVE")
                )
                label = (
    ...
    with tab_live:
        ...
        for idx, direction in enumerate(LANES):
            ...
            tracking_cols[idx].metric(
                ...
                help="Vehicles confirmed as persistent queue members after several low-motion checks.",
            )
