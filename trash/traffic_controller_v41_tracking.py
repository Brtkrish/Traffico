...
def update_centroid_tracks(direction, detections, now_ts):
    """
    Match current detection centroids to the previous frame using nearest-neighbor
    association. This is intentionally lightweight so it works with the existing
    YOLO model without requiring a second tracking model.

    detections: list of dicts with cx, cy, bbox, cls, conf.
    Returns the same detections with track_id / speed / stopped fields.
    """
    ...
    dt = 1.0 / max(st.session_state.video_fps[direction], 1.0)
    if previous_ts is not None:
        dt = max(0.01, now_ts - previous_ts)
    ...
    for det in detections:
        ...
        for tid in unmatched_track_ids:
            ...
            if dist < best_dist and dist <= TRACK_MAX_DISTANCE_PX:
                best_dist = dist
                best_id = tid
        ...
        if best_id is None:
            ...
            # Smooth the speed so one noisy detection doesn't flip the state.
            tr["speed"] = 0.65 * tr["speed"] + 0.35 * pixel_speed
            tr["age"] += 1
            tr["missed"] = 0
            tr["last_seen"] = now_ts
            tr["cx"] = det["cx"]
            ...
            if tr["age"] >= MIN_TRACK_AGE:
                tr["stopped"] = tr["speed"] <= STOP_SPEED_PX_PER_SEC
    ...
    # Age tracks that disappeared.
    for tid in list(unmatched_track_ids):
        tracks[tid]["missed"] += 1
        if tracks[tid]["missed"] > TRACK_MAX_MISSED:
            del tracks[tid]

    st.session_state.next_track_id[direction] = next_id
    st.session_state.last_processed_time[direction] = now_ts

    active = [tr for tr in tracks.values() if tr["missed"] == 0]
    stopped = [tr for tr in active if tr["age"] >= MIN_TRACK_AGE and tr["stopped"]]
    avg_speed = (
        sum(tr["speed"] for tr in active) / len(active)
        if active else 0.0
    )
    ...
    with tab_live:
        ...
        for idx, direction in enumerate(LANES):
            ...
            tracking_cols[idx].metric(
                ...
                help="Vehicles currently tracked as stopped inside this approach ROI.",
            )
        ...
        for direction, slot in sig_slots.items():
            ...
            track_info = st.session_state.track_metrics.get(
                direction,
                {"stopped": 0, "queue": 0, "avg_speed_px_s": 0.0},
            )
